...
void test2() {
  ...
  memcpy(bigbuffer, smallbuffer, sizeof(smallbuffer)); // GOOD
  memcpy(bigbuffer, smallbuffer,
         sizeof(bigbuffer)); // BAD: over-read [NOT DETECTED]
  memcpy(smallbuffer, bigbuffer, sizeof(smallbuffer)); // GOOD
  memcpy(smallbuffer, bigbuffer,
         sizeof(bigbuffer)); // BAD: over-write [NOT DETECTED]
...
void test9(int param) {
...

  {
    ...
    strncpy(str2, str1,
            strlen(str2) + 1); // BAD: overrun read of str1 [NOT REPORTED]
  }

  memmove(global_array_6, global_array_5,
          6); // BAD: overrun read of global_array_5
  memmove(global_array_5, global_array_6,
          6); // BAD: overrun write of global_array_5
  ...
  if (param > 0) {
    ...
    memset(buffer, 0xFF,
           param * 2); // BAD: overrun write of buffer [NOT REPORTED]
  }
}
...
void test12() {
  ...
  memset(&(myVar.field), 0,
         sizeof(int) * 2); // BAD: overrun write of myVar.field
...
void test16() {
  ...
  int *ptr1_3 = &arr1[11]; // BAD: not guaranteed to be valid [NOT DETECTED -
                           // considered outside the scope of CWE-119]
  ...
  mystruct16 *ptr2_2 =
      &arr2[10]; // GOOD: valid as long as we don't dereference it
  mystruct16 *ptr2_3 =
      &arr2[11]; // BAD: not guaranteed to be valid [NOT DETECTED - considered
                 // outside the scope of CWE-119]
...
void test17(long long *longArray) {
...

  {
    ...
    multi[5][-5] = 0;  // DUBIOUS: underrun write (this one is still within the
                       // bounds of the whole array)
    multi[-5][-5] = 0; // BAD: underrun write [INCORRECT MESSAGE]
    multi[0][-5] = 0;  // BAD: underrun write [NOT DETECTED]

    multi[15][5] = 0;  // BAD: overrun write
    multi[5][15] = 0;  // DUBIOUS: overrun write (this one is still within the
                       // bounds of the whole array)
...
void test21(bool cond) {
  ...
  if (ptr[-1] == 0) {
    return;
  } // BAD: accesses buffer[-1] or buffer[0]
